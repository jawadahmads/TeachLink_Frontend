...
export function AddAvailabilityDialog({
  ...
  const [activePreset, setActivePreset] = useState<keyof typeof TIME_PRESETS | null>(null);
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {(Object.keys(TIME_PRESETS) as Array<keyof typeof TIME_PRESETS>).map((presetKey) => {
                ...
                return (
                  <Button
                    ...
                    className={`h-10 px-4 rounded-xl font-bold transition-all ${
                      isActive ? "bg-primary shadow-lg shadow-primary/20" : "border-2"
                    }`}
                  >
                    <Icon className="h-4 w-4 mr-2" />
                    {preset.label}
                    <ChevronDown className={`h-4 w-4 ml-2 transition-transform ${isActive ? "rotate-180" : ""}`} />
                  </Button>
                );
              })}
            </div>
              ...
              <div className="bg-muted/50 rounded-2xl p-4 space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                <div className="flex justify-between items-center pt-2">
                  <span className="text-sm font-semibold text-muted-foreground">
                    {tempSlots.filter(t => TIME_PRESETS[activePreset].times.includes(t)).length} selected
                  </span>
                  <Button
                    type="button"
                    variant="ghost"
