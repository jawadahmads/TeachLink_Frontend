    ...
    <Card className="border border-border/10 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] bg-card/40 backdrop-blur-3xl rounded-[48px] overflow-hidden group">
      ...
      <CardContent className="p-8 pt-0 space-y-4">
        {[
          ...
          <div
            key={i}
            className="flex items-center justify-between p-5 rounded-[24px] bg-background/20 border border-border/10 hover:border-primary/20 hover:bg-primary/5 transition-all duration-500"
          >
            <span className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-widest">
              {item.label}
            </span>
            <span className="font-black text-primary text-lg tracking-tighter">
              {item.value}
            </span>
          </div>
        ))}
